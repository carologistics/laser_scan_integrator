...
scanMerger::scanMerger() : Node("laser_scan_integrator"), last_call_time_(now()) {
    ...
    auto default_qos = rclcpp::
        SensorDataQoS(); // rclcpp::QoS(rclcpp::SystemDefaultsQoS());
    sub1_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        topic1_, default_qos,
        std::bind(&scanMerger::scan_callback1, this,
                  std::placeholders::_1));
    sub2_ = this->create_subscription<sensor_msgs::msg::LaserScan>(
        topic2_, default_qos,
        std::bind(&scanMerger::scan_callback2, this,
                  std::placeholders::_1));
    ...
    line_segments_pub_ = this->create_publisher<
        laser_scan_integrator_msg::msg::LineSegments>(
        "line_segments", rclcpp::SystemDefaultsQoS());

    segmentation_enabled_ = false;

    segmentation_service_ = this->create_service<
        laser_scan_integrator_msg::srv::ToggleSegmentation>(
        "toggle_segmentation",
        std::bind(&scanMerger::handle_toggle_segmentation, this,
...
void scanMerger::handle_toggle_segmentation(
...
// Create point cloud
pcl::PointCloud<pcl::PointXYZ>::Ptr scanMerger::laser_scan_to_pointcloud(
    ...
    for (size_t i = 0; i < scan->ranges.size(); ++i) {
        ...
        if (range >= scan->range_min && range <= scan->range_max) {
            ...
            cloud->points.emplace_back(range * std::cos(angle),
                                       range * std::sin(angle), 0.0f);
        }
    }
    return cloud;
}

void scanMerger::publishPointCloud(const sensor_msgs::msg::LaserScan::SharedPtr &scan) {
...
scanMerger::calc_lines(typename pcl::PointCloud<pcl::PointXYZ>::ConstPtr input,
           unsigned int segm_min_inliers,
           unsigned int segm_max_iterations,
           float segm_distance_threshold,
    ...
    typename pcl::PointCloud<pcl::PointXYZ>::Ptr in_cloud(
    ...
    while (in_cloud->points.size() > segm_min_inliers) {
        ...
        //   RCLCPP_INFO(this->get_logger(), "  inliers->indices[%zu] = %d",
        //   i,
        //               inliers->indices[i]);
        // }
        ...
        if ((double)inliers->indices.size() < segm_min_inliers) {
            // logger->log_warn(name(), "[L %u] no more lines (%zu inliers,
            // required %u)", 	       loop_count_, inliers->indices.size(),
            // segm_min_inliers);
            break;
        }
        ...
        // the line search can output a line which combines lines at
        // separate ends of the field of view...

        typename pcl::search::KdTree<pcl::PointXYZ>::Ptr
            kdtree_line_cluster(new pcl::search::KdTree<pcl::PointXYZ>());
        ...
        size_t min_size =
            (size_t)floorf(cluster_quota * inliers->indices.size());
        ...
        if (!line_cluster_indices.empty()) {
            line_cluster_index = pcl::PointIndices::Ptr(
        ...
        if (line_cluster_index) {
            ...
            *line_cluster_index = *tmp_index;
            // RCLCPP_INFO(this->get_logger(),
            //             "tmp_index (Anzahl: %zu):",
            //             tmp_index->indices.size());
            // for (std::size_t i = 0; i < tmp_index->indices.size(); ++i) {
            //   RCLCPP_INFO(this->get_logger(), "  tmp_index->indices[%zu]
            //   = %d", i,
            //               tmp_index->indices[i]);
            // }
        }
        ...
        typename pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_f(
            new pcl::PointCloud<pcl::PointXYZ>());
        ...
        *deduped = *cloud_f;
        ...
        // Point on the line
        Eigen::Vector3f point_on_line(coeff->values[0], coeff->values[1],
                                      coeff->values[2]);
        // Direction vector of the line
        Eigen::Vector3f direction(coeff->values[3], coeff->values[4],
                                  coeff->values[5]);
        direction.normalize();
        ...
        // if (line_length == 0 || (min_length >= 0 && line_length <
        // min_length)
        //                   || (max_length >= 0 && line_length >
        //                   max_length)) {
        if (line_length == 0 ||
            (line_length < 0.65 || line_length > 0.75)) {
            continue;
        }
...
// Methode zum Erstellen und sofortigen Publizieren von Linien-Markern in
// RViz
void scanMerger::publishLineMarkers(
...
void scanMerger::update_point_cloud_rgb() {
    ...
    trans2_ = tf2_->lookupTransform(
        integratedFrameId_, laser2_->header.frame_id, rclcpp::Time(0));
    ...
    if (show1_) {
        ...
        for (float i = laser1_->angle_min; i <= laser1_->angle_max;
            ...
            pt[0] =
                temp_x * std::cos(sensor1_y) - temp_y * std::sin(sensor1_y);
            pt[0] += trans1_.transform.translation.x + laser1XOff_;
            pt[1] =
                temp_x * std::sin(sensor1_y) + temp_y * std::cos(sensor1_y);
    ...
    if (show2_) {
        for (float i = laser2_->angle_min; i <= laser2_->angle_max;
            ...
            pt[0] =
                temp_x * std::cos(sensor2_y) - temp_y * std::sin(sensor2_y);
            pt[0] += trans2_.transform.translation.x + laser2XOff_;
            pt[1] =
                temp_x * std::sin(sensor2_y) + temp_y * std::cos(sensor2_y);
    ...
    for (float angle = min_theta; angle < max_theta;
        ...
        if (fabs(scan_data[i][1] - scan_data[i - 1][1]) < 0.2f &&
            (fabs(scan_data[i][0] - angle) < laser1_->angle_increment ||
             fabs(scan_data[i - 1][0] - angle) <
                 laser1_->angle_increment)) {
            float range =
                interpolate(scan_data[i - 1][0], scan_data[i][0],
                            scan_data[i - 1][1], scan_data[i][1], angle);
            if (range <= RangeMin_) {
                temp_range.push_back(
    ...
    if (segmentation_enabled_) {
        ...
        //  Detect lines in the point cloud with a target length of 70 cm
        //  and a tolerance of 2 cm
        ...
        lines_msg.header =
            integrated_msg_->header; // Retain the original message header
                                     // for consistency
...
float scanMerger::GET_R(float x, float y) { return sqrt(x * x + y * y); }

float scanMerger::GET_THETA(float x, float y) { return atan2(y, x); }

float scanMerger::interpolate(float angle_1, float angle_2, float magnitude_1,
...
void scanMerger::geometry_quat_to_rpy(double *roll, double *pitch, double *yaw,
...
void scanMerger::refresh_params() {
    this->get_parameter_or<std::string>("integratedTopic", integratedTopic_,
                                        "/robotino3base1/scan");
    this->get_parameter_or<std::string>("integratedFrameId",
                                        integratedFrameId_, "laser_link");
    this->get_parameter_or<std::string>(
        "scanTopic1", topic1_, "/robotino/SickLaser_Front_Remaped");
